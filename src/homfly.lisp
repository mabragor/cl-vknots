  ...
  (let ((type (what-object-is-it thing)))
    (cond ((eq :planar-diagram type)
	  ...
	  ((eq :serialized-dessin type)
	   ...
	   (if (< 1 cable)
	       (format t "Cabling is not well-defined for dessins, calculating fundamental answer~%"))
	   (homfly-actual-serial-toolchain thing))
	  ((eq :prehorde type)
	   (format t "I think, you've entered horde diagram in form of edge numbers, calculating ...~%")
	   (if (< 1 cable)
	       (format t "Cabling is not well-defined for horde-diagrams, calculating fundamental answer~%"))
	  ...
	  ((eq :mathematica-thing type)
	   ...
	   (if (< 1 cable)
	       (homfly-actual-serial-toolchain
		(planar->seifert (link-free-cable (braid->planar (get-braid-rep1 thing))
						  cable)))
	       (homfly-actual-serial-toolchain
		(planar->seifert (braid->planar (get-braid-rep1 thing))))))
	  ((eq :i-dont-know type)
